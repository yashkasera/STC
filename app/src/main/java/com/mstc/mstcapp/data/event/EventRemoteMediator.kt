...
private const val TAG = "EventRemoteMediator"
...
class EventRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Event>): MediatorResult {
        ...
        try {
            val response = service.getEvent(page)
                ...
                events?.let { it ->
                    ...
                    eventDatabase.withTransaction {
                        ...
                        if (loadType == LoadType.REFRESH) {
                            eventDatabase.eventKeyDao().clearAll()
                            eventDatabase.eventDao().clearAll()
                        }
                        val prevKey = if (page == FEED_STARTING_PAGE_INDEX) null else page - 1
                        val nextKey = if (endOfPaginationReached == true) null else page + 1
                        val keys = events.map {
                            EventRemoteKey(eventId = it.id,
                                prevKey = prevKey,
                                nextKey = nextKey)
                        }
                        eventDatabase.eventKeyDao().insertAll(keys)
                        eventDatabase.eventDao().insertAll(it)
                    }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Event>): EventRemoteKey? {
        ...
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { event ->
                ...
                eventDatabase.eventKeyDao().eventRemoteKeysId(event.id)
            }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, Event>): EventRemoteKey? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { eventId ->
                eventDatabase.eventKeyDao().eventRemoteKeysId(eventId)
            }
        }
    }
}
